...
class OrderDB:
    ...
    def connect(self):
        ...
        try:
            ...
            self.cnx = mysql.connector.connect(user='ksorense', password='28669364', host='cosc304.ok.ubc.ca', database='workson')
            return self.cnx
    ...
    def listAllCustomers(self):
        ...
        cursor.execute(query)for (CustomerId, CustomerName) in cursor:print(CustomerId, CustomerName):
    ...
    def listCustomerOrders(self, customerId):
        ...
        print("Executing list all orders for a customer.")
        
        output = "OrderId, OrderDate, CustomerId, EmployeeId, Total"
                
        cursor = self.cnx.cursor()
        # TODO: Execute query and build output string 
        query = "SELECT OrderId, ename, salary FROM emp WHERE salary < 50000"
        cursor.execute(query)for (eno, ename, salary) in cursor:print(eno, ename, salary)
